...
/*
		$scope.exh.$on('loaded', checkData);
      		$scope.exh.$on('change', checkData);

		function checkData() {
		      	if ($scope.item.amount_paid == $scope.exh.forcast_revenue) {
				$scope.paymentExp = {label: "success",  icon: "check"};
				$scope.methodExp = true;
				$scope.duesExp = false;
			} else if($scope.exh.amount_paid  !== "0.00"){
				$scope.paymentExp = {label: "warning",  icon: "refresh"};
				$scope.methodExp = true;
				$scope.duesExp = $scope.exh.forcast_revenue - $scope.exh.amount_paid;
			} else {
				$scope.paymentExp = {label: "danger",  icon: "times"};
				$scope.methodExp = false;
				$scope.duesExp = $scope.exh.forcast_revenue;
			};
...
.controller("sortableController", ['$firebase', 'Firebase', '$scope', 'syncData', function($firebase, Firebase, $scope, loginService, syncData){
	...
	$scope.init = function(tile) {
		...
		$scope.messages = syncData('users' , 10);
		// console.log($scope.messages);
		$scope.listddd = tmpList;
		$scope.hold = [
			{ 'text': 'L', 'drag': true },
			{ 'text': 'O', 'drag': true },
			{ 'text': 'M', 'drag': true },
